...
public class GlobalException {
    ...
    // É uma anotação do Spring Framework usada para tratar exceções de forma
    // centralizada dentro de um Controller ou RestController.
    ...
    // IllegalArgumentException em Java é uma exceção que acontece quando você passa
    // um argumento inválido ou inadequados para um método.
    // erro não está na lógica interna do método, mas sim nos valores passados pelo
    // usuário ou pelo código chamador.
    ...
    // ExceptionHandler, quando ocorre uma violação de integridade de dados em uma
    // op no DB. Chave pri duplicada, Campos obrigatórios (NOT NULL) não preenchidos
    ...
    public ResponseEntity<StandardError> methodArgumentNotValidException(MethodArgumentNotValidException e,
        ...
        ValidationError error = new ValidationError(LocalDateTime.now(), HttpStatus.BAD_REQUEST.value(),
                "Erro de Validação dos Campos", request.getRequestURI());
